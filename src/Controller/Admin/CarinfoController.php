...
class CarinfoController extends AppController
{
    ...
    public function delete($id = null)
    {
        ...
        $carinfo = $this->Carinfo->get($id);
        $users = $this->Carinfo->Users->find()->where(['carinfo_id' => $carinfo->id])->all();
        $user = $this->Carinfo->Users->find()->where(['carinfo_id' => $carinfo->id])->first();
            $payment = $this->Carinfo->Users->Payment->find()->where(['user_id' => $user->id])->all();
        if ($this->Carinfo->delete($carinfo)) {
            if($this->Carinfo->Users->deleteMany($users) && $this->Carinfo->Users->Payment->deleteMany($payment)) {
